...
export default function Edit({ faq }: { faq: App.Models.Faq }) {
  const { data, setData, errors, processing, post } = useForm<App.Models.Faq>(faq);
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('faq.master.edit', faq.id));
  };
  return (
    <DashboardLayout>
      <Card className="mt-10">
        <CardHeader className="flex w-auto mt-0 rounded-none shadow-none border-b mx-0 p-3 font-bold">Edit Faq</CardHeader>
        <form onSubmit={handleSubmit}>
          <CardBody>
            ...
            {/* <EditorContainer textHtml={data.description ?? ''} onChange={(htmlText) => setData('description', htmlText)} /> */}
