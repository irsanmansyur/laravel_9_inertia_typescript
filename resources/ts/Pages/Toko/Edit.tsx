...
export default function Edit({ toko }: { toko: resourceLaraveInterface<App.Models.Toko> }) {
  const refPreview = useRef<HTMLImageElement>(null);
  const { data, setData, errors, post, processing } = useForm<App.Models.Toko>({ name: toko.data.name, status: toko.data.status, link: toko.data.link, logo: "" });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route("toko.master.edit", toko.data.id));
  };
  useEffect(() => {
    if (typeof toko.data.logo != "string" || !refPreview.current) return;
    refPreview.current.src = toko.data.logo;
    refPreview.current.style.display = "block";
  }, []);
  return (
    <DashboardLayout>
        ...
        <form onSubmit={handleSubmit}>
          <CardBody>
            ...
            <div className="-ml-2 mt-2">
              <Checkbox name={"status"} label="Active ? " onChange={onHandleChange} checked={data.status === "active"} />
